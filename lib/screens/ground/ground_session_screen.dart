...
const List<GuidedTrack> kGuidedTracks = [
  GuidedTrack(
    id: '01',
    title: 'Come to Presence',
    subtitle: 'arriving here, now',
    durationMinutes: 5,
    assetPath: 'assets/audio/G01_phase01.mp3',
  ),
  GuidedTrack(
    id: '02',
    title: 'Cause & Effect I',
    subtitle: 'seeing what moves within',
    durationMinutes: 8,
    assetPath: 'assets/audio/G01_phase02.mp3',
  ),
  GuidedTrack(
    id: '03',
    title: 'Coming Soon',
    subtitle: 'more transmissions will arrive',
    durationMinutes: 0,
  ),
  GuidedTrack(
    id: '04',
...
class _GroundSessionScreenState extends State<GroundSessionScreen>
  ...
  // Track 01 is initially available.
  // Track 02 becomes available after Track 01 completes.
  // Tracks 03–10 are intentionally unavailable in this release.
  ...
  void initState() {
    ...
    _breathController = AnimationController(
      vsync: this,
      duration: Duration(
        seconds: _kBreathDurations[_BreathPhase.inhale]!,
      ),
    );
    ...
    if (widget.mode == SessionMode.guided &&
      ...
      _showTrackList = true;
      _loadTrackProgress();
    } else if (widget.mode == SessionMode.guided &&
        widget.selectedTrack != null) {
      _activeTrack = widget.selectedTrack;
      _totalSessionSeconds =
          _activeTrack!.durationMinutes * 60;
    } else {
  ...
  Future<void> _loadTrackProgress() async {
    ...
    final savedMax = prefs.getInt(
      'ground_max_unlocked_track',
    ) ?? 1;

    if (!mounted) return;

    setState(() {
      _maxUnlockedTrack = savedMax.clamp(1, 1);
    });
  }
  ...
  Future<void> _unlockNextTrack() async {
    ...
    final currentIndex = kGuidedTracks.indexWhere(
      (track) => track.id == _activeTrack!.id,
    );

    // Only Track 01 can unlock Track 02 in this release.
  ...
  void _selectTrack(GuidedTrack track) {
    if (kGuidedTracks.indexOf(track) > 1) return;
    if (track.assetPath == null) return;
  ...
  void _startSessionTimer() {
    ...
    _sessionTimer = Timer.periodic(
      ...
      (timer) {
        if (!mounted || _isPaused || _isEnding) return;
        ...
        setState(() {
          ...
          final phaseDuration =
              _kBreathDurations[_phase]!;

          if (_phaseSeconds >= phaseDuration) {
            _phaseSeconds = 0;

            const phases = _BreathPhase.values;
            final next =
                phases[(_phase.index + 1) % phases.length];
  ...
  void _handleAudioCompleted() {
    ...
    if (_autoAdvance) {
      ...
      if (current != null) {
        ...
        final canAdvanceToSecondTrack =
            currentIndex == 0 &&
            nextIndex == 1 &&
            _maxUnlockedTrack >= 1 &&
            kGuidedTracks[nextIndex].assetPath != null;

        if (canAdvanceToSecondTrack) {
          final next = kGuidedTracks[nextIndex];
  ...
  Future<void> _endSession() async {
    ...
    try {
      ...
      await QuickAudio.player.setLoopMode(LoopMode.off);
    } catch (e) {
      debugPrint('GROUND AUDIO STOP ERROR: $e');
    }
    ...
    setState(_resetForNextSession);

    widget.onComplete();
  }

  // Manual exit: triggered by the END button, or a back gesture/press
  // while a session is active. Unlike natural completion, this does
  // NOT count as a finished session — no growth, no completion screen.
  // Audio is paused (not torn down) and the user is returned to the
  // track list (guided) or the prep screen (free).
  Future<void> _exitSessionEarly() async {
    if (_isEnding) return;
    ...
    try {
      await QuickAudio.player.pause();
      await QuickAudio.player.setLoopMode(LoopMode.off);
    } catch (e) {
      debugPrint('GROUND AUDIO PAUSE ERROR: $e');
    }
  ...
  void _runBreathPhase(_BreathPhase phase) {
    if (!mounted || _isEnding || _isPaused) return;

    final duration = Duration(
      seconds: _kBreathDurations[phase]!,
    );
  ...
  void _resetForNextSession() {
    ...
    _audioPosition = Duration.zero;
    _audioDuration = Duration.zero;
    _showTrackList = widget.mode == SessionMode.guided;
    _isEnding = false;
  }
  ...
  String get _timeRemaining {
    final remaining = max(
      0,
      _totalSessionSeconds - _sessionSeconds,
    );
  ...
  void dispose() {
    ...
    // Safety net: if the screen is left via the system back
    // gesture/button instead of the END button, make sure any
    // active guided/free-session audio doesn't keep playing.
    if (_sessionStarted && !_isEnding) {
      QuickAudio.stop();
    }
  ...
  Widget build(BuildContext context) {
    ...
    if (_showTrackList) {
      content = _buildTrackListScreen();
    } else if (!_sessionStarted) {
      content = widget.mode == SessionMode.free
          ? _buildFreePrepScreen()
          : _buildGuidedPrepScreen();
    } else {
      content = _buildActiveSession();
    }
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              ...kSoundEnvironments.map(
                (env) {
                  final selected =
                      _selectedEnv.id == env.id;
